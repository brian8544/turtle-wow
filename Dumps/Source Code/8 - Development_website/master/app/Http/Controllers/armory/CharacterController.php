...
class CharacterController extends Controller
{
    ...
    public function getCharacter()
    {
        ...
        $characterItems = DB::connection($realmId)->table('character_inventory')
            ->selectRaw("itemEntry, slot, bag, item_instance.enchantments, item_instance.randomPropertyId ")
        ...
        foreach ($characterItems as $characterItem) {
            ...
            $characterItem->subclassId = $finalItems[$characterItem->itemEntry]['subclass'];
            $characterItem->gems = [];
        ...
        if ($character->ammoId != 0) {
            ...
            $ammoItem->subclassId = $finalItems[$character->ammoId]['subclass'];
            $ammoItem->gems = [];
    ...
    public function getItems()
    {
        return Cache::remember('armoryItems', 60, function () {
            ...
            $items = ItemTemplate::select(['entry', 'class', 'subclass', 'quality', 'display_id', 'name', 'inventory_type'])->get();
            $finalItems = [];
